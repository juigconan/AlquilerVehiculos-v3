...
public class BuscarVehiculo extends Controlador {
	...
	private void buscar(ActionEvent actionEvent) {
		...
		tcPlazas.setCellValueFactory(
				fila -> new SimpleObjectProperty<String>(VentanaPrincipal.calcularPlazas(fila.getValue())));
		tcPma.setCellValueFactory(
				fila -> new SimpleObjectProperty<String>(VentanaPrincipal.calcularPma(fila.getValue())));
		tcCilindrada.setCellValueFactory(
				fila -> new SimpleObjectProperty<String>(VentanaPrincipal.calcularCilindrada(fila.getValue())));
		try {
			tvVehiculo.getItems().add(vista.getControlador().getVehiculos().get(vista.getControlador().getVehiculos()
					.indexOf(Vehiculo.getVehiculoConMatricula(tfMatricula.getText()))));
